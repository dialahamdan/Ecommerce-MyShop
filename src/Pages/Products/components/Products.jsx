...
<div className="row mb-3">
  <div className="col-md-4">
    <select defaultValue="Sort Options" className=" Filter" onChange={(e) => getProductsSorted(currentPage, e.target.value, e)}>
  ...
  <div className="col-md-4">
    <form onSubmit={(e)=>{e.preventDefault();getProductsByMin(currentPage,min,max)}} className="mb-3 d-flex">
      ...
      <button type="submit" className="searchBtn ">Get</button>
      <button className=" bg-danger" onClick={ResetInputs}>Clear</button>
    </form>
  </div>
</div>
            ...
            <div className='col-md-4' key={product._id}>
               <div className='allProducts'>
                <div className='everyProduct text-center '> 
                   <img className='pic mx-auto' src={product.mainImage.secure_url} alt={product.name} /> 
                  <h2 className='fs-6' >{product.name}</h2>
                  <Link className='detailsBtn' to={`/ProductsDetails/${product._id}`}>Details</Link>
                 </div>
               </div>
             </div>
          ))}
